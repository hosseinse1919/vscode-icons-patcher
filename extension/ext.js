...
function MakeExistAndCopy(params) {
...
function commitchanges(requestpath) {
    fs_1.readdirSync(requestpath).forEach(fdr => {
        if (fdr.startsWith("robertohuertasm.vscode-icons")) {
            var localpth = path_1.join(requestpath, fdr, "out", "src");
            ...
            console.log("vscode-icons updated", requestpath);
            MakeExistAndCopy(localpth);
            UpdateData();
        }
    });
}
...
if (hpath[0]) {
    vsnames.forEach(p => {
        var extfolder = path_1.join(hpath[1], "." + p, "extensions");
        commitchanges(extfolder);
    });
}
